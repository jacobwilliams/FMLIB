...
!  Test version 1.4 of FM_parallel.f95, the thread-safe version of the FM package, using OpenMP.
  ...
  USE OMP_LIB
    ...
    END INTERFACE
      ...
      TYPE (FM) :: X1, X2, R, X_VALS(2*N_VALS), A(5,5), B(5), X(5), DET, AINV(5,5),  &
                   AX, BX, TOL, XVAL, FVAL, S(3), S1(3)
      TYPE (IM) :: I1
      TYPE (ZM) :: Z1, Z2, Z3, ZA(5,5), ZB(5), ZX(5), ZDET, ZAINV(5,5), LIST_OF_ROOTS(8)
      INTEGER :: I, J, K, N_CASE, N_FOUND, N_ORDER, N_FUNCTION
      INTEGER :: L, N_ERRORS, SKIP, SKIP_CHECK(30), THREADS_USED, TOTAL_CASES
      CHARACTER(8) :: FILE_NUMBER
      CHARACTER(25) :: FILE_NAME
      CHARACTER(251) :: LINE1, LINE2
      LOGICAL :: CHK_FILE_EXISTS
      DOUBLE PRECISION :: TIME1, TIME2, RM, RAN(4*N_VALS*MAX_CASE)
...
!$OMP PARALLEL PRIVATE(I,J,K,N_CASE,N_FOUND,N_ORDER,N_FUNCTION,X1,X2,R,A,B,X,DET,AINV,AX,BX,  &
!$OMP &                TOL,XVAL,FVAL,S,S1,I1,Z1,Z2,Z3,ZA,ZB,ZX,ZDET,ZAINV,LIST_OF_ROOTS,      &
!$OMP &                FILE_NUMBER,FILE_NAME)                      &
!$OMP &        SHARED(TOTAL_CASES,RAN,THREADS_USED,X_VALS)
...
!$OMP BARRIER
      ...
      DO I = OMP_GET_THREAD_NUM()+1, 2*N_VALS, OMP_GET_NUM_THREADS()

         IF (I == OMP_GET_THREAD_NUM()+1) THEN
             WRITE(FILE_NUMBER,"(I8)") I
             FILE_NAME = TRIM( 'TestFM_parallel' // ADJUSTL(FILE_NUMBER) ) // '.out'
             OPEN(31+OMP_GET_THREAD_NUM(),FILE=TRIM(FILE_NAME))
         ENDIF
...
!             don't call it an error if one of those cases differs from the chk file.
      ...
      DO K = 1, 2*N_VALS
         ...
         ENDDO
         I = I + 1
         IF (I > THREADS_USED) I = 1
      ENDDO
      WRITE (*,"(//I5,A,I6,A)") TOTAL_CASES*2*N_VALS, ' cases were tested using',  &
  ...
  USE OMP_LIB
      ...
      WRITE (31+OMP_GET_THREAD_NUM(),"(I4,I6,5X,A)") I,N_CASE,TRIM(ST1)
